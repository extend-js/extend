...
export function reflectionTitle(this: PageEvent, shouldEscape = true): string {
  const title: string[] = [];
  ...
  if (this.model.typeParameters) {
    const typeParameters = this.model.typeParameters.map((typeParameter: ParameterReflection) => typeParameter.name).join(', ');
